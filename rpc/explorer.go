...
type explorerMempoolResponse struct {
	Count        int                           `json:"count"`
	Transactions []explorerTransactionResponse `json:"transactions"`
}

var explorerDataSources = []string{"seed-1", "seed-2", "mempool", "demand-miner"}
...
func (s *Server) handleExplorerStatus(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, explorerStatusResponse{
		...
		DataSources:  explorerDataSources,
	})
}
...
func (s *Server) handleExplorerMempool(w http.ResponseWriter, r *http.Request) {
	...
	views := make([]explorerTransactionResponse, 0, len(txs))
	for _, tx := range txs {
		if tx == nil {
			continue
		}
		views = append(views, explorerTransactionResponse{
			Transaction: explorerTransaction(tx),
			Status:      "pending",
		})
	}
	writeJSON(w, http.StatusOK, explorerMempoolResponse{
