...
func TestPoWPolicyForNetworkRemainsDisabled(t *testing.T) {
	for _, network := range []params.NetworkID{
		...
		if policy.GPUV1ActivationHeight != params.GPUV1ActivationDisabled {
			t.Fatalf(
				"network %q activation = %d, want disabled sentinel %d",
				network,
...
func TestPoWPolicyVersionMatrix(t *testing.T) {
	cases := []struct {
		...
		{name: "disabled legacy maximum height", activation: params.GPUV1ActivationDisabled, height: ^uint64(0), version: 1, allowed: true},
...
func TestPoWPolicyForNetworkRejectsUnknownNetwork(t *testing.T) {
	if _, err := PoWPolicyForNetwork(params.NetworkID("unknown")); err == nil {
		t.Fatal("unknown network unexpectedly returned a PoW policy")
	}
}
