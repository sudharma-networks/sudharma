...
func CloneChain(source *Chain) (*Chain, error) {
	...
	return &Chain{
		blocks: blocks,
		totalWork: new(big.Int).Set(
			source.totalWork,
		),
	}, nil
}

// ReplaceWith atomically replaces the current chain with
// another already validated chain.
//
// This does NOT decide which chain is better.
// BetterChain must be used before replacement.
func (c *Chain) ReplaceWith(
	candidate *Chain,
) error {
	...
	if newBlocks[0] == nil {
		return fmt.Errorf(
			"candidate genesis block is nil",
		)
	}

	if newBlocks[0].Hash() !=
		expectedGenesis.Hash() {

		return fmt.Errorf(
			"candidate has wrong genesis block",
		)
	}

	// Revalidate the complete candidate sequence before commit.
...
func ReorganizeToCandidate(
	...
	if err != nil {
		return false, fmt.Errorf(
			"candidate state rebuild failed: %w",
			err,
		)
	}
	...
	if err :=
		...
		_ = current.ReplaceWith(
			originalChain,
		)

		_ = currentState.ReplaceWith(
			originalState,
		)

		return false, fmt.Errorf(
			"failed replacing blockchain state: %w",
			err,
		)
	}
