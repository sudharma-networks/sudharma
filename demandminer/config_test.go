...
func TestConfigRejectsMissingFields(t *testing.T) {
	cfg := validConfig()
	cfg.RewardAddress = ""
	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "reward_address") {
		t.Fatalf("expected missing reward_address rejection, got %v", err)
	}
}
...
func TestConfigRejectsMainnetIdentity(t *testing.T) {
	...
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected testnet-only rejection")
	}
}

func TestConfigRejectsInvalidRewardAddress(t *testing.T) {
	cfg := validConfig()
	cfg.RewardAddress = strings.Repeat("g", 40)
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected invalid reward address rejection")
	}
	cfg.RewardAddress = strings.Repeat("a", 39)
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected reward address length rejection")
	}
}
...
func TestConfigRejectsInvalidDurations(t *testing.T) {
	for _, field := range []string{"PollEvery", "Cooldown", "FailureBackoff"} {
		...
		case "Cooldown":
			cfg.Cooldown = "-1s"
		case "FailureBackoff":
			cfg.FailureBackoff = "nonsense"
		}
		if err := cfg.Validate(); err == nil {
			t.Fatalf("expected invalid %s rejection", field)
		}
	}
}
...
func TestConfigRejectsCooldownShorterThanPoll(t *testing.T) {
	...
	cfg.PollEvery = "20s"
	cfg.Cooldown = "10s"
	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "cooldown") {
...
func TestConfigRejectsRelativePaths(t *testing.T) {
	cfg := validConfig()
	cfg.MinerBinary = "./sudharmad"
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected relative miner binary rejection")
	}
	cfg = validConfig()
	cfg.DataDirectory = "data"
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected relative data directory rejection")
	}
}

func TestConfigRejectsChildTimeoutBelowOneSecond(t *testing.T) {
	cfg := validConfig()
	cfg.ChildTimeout = "999ms"
	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "child_timeout") {
		t.Fatalf("expected child timeout rejection, got %v", err)
	}
}
...
func TestLoadConfigRejectsUnknownFieldsAndParsesDurations(t *testing.T) {
	...
	if got, err := cfg.PollDuration(); err != nil || got != 10*time.Second {
		t.Fatalf("PollDuration = %v, %v", got, err)
	}
}
