...
func TestConfigValidateRejectsUnsafeValues(t *testing.T) {
	tests := []struct {
		...
		{"nonnumeric seed port", func(c *Config) { c.SeedAddress = "127.0.0.1:notaport" }, "port"},
		{"zero seed port", func(c *Config) { c.SeedAddress = "127.0.0.1:0" }, "port"},
		{"out of range seed port", func(c *Config) { c.SeedAddress = "127.0.0.1:65536" }, "port"},
