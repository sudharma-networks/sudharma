#!/usr/bin/env bash
# Builds linux/amd64 sudharma-rpcd and uploads it to S3 for seed upgrades.
set -euo pipefail
...
(
  ...
  GOOS=linux GOARCH=amd64 CGO_ENABLED=0 \
    go build -trimpath -ldflags="-s -w" -o "$out_dir/sudharma-rpcd" ./cmd/sudharma-rpcd
)
...
jq -nc \
  --arg bucket "$bucket" \
  --arg prefix "$prefix" \
  --arg rpcd_url "$rpcd_url" \
  '{bucket:$bucket,prefix:$prefix,sudharma_rpcd_bin_url:$rpcd_url}'
