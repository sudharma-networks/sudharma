...
if ! grep -Fq -- 'cursor/canonical-integration-guard-8441' .github/workflows/faucet-recovery-ci.yml \
  && ! grep -Fq -- 'fix/faucet-health-timeout' .github/workflows/faucet-recovery-ci.yml; then
  fail 'canonical integration branch must run non-deploying recovery CI'
fi
