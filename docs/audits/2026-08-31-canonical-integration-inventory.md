...
| --- | --- | --- | --- |
| Faucet recovery, demand miner, and explorer deployment | `feature/faucet-recovery-stage2` at `2e2abd6786309fc1f4817e76b257eb632bd10fdc` | 159 commits ahead; recorded `main` is an ancestor | Preferred source for this overlapping slice; PR #66 safety changes are included |
| Android wallet, faucet client, and earlier demand miner reconciliation | `codex/canonical-wallet-integration` at `6096172ef74775cf9aa68c32d0efba143400f61d` | 253 commits ahead and 82 behind | Source only for files absent from the recovery line; PR #26 is conflicting and must not be merged wholesale |
...
another. Their common files therefore require a file-level comparison; branch
merges or broad commit-range cherry-picks are not acceptable substitutes.

## Integration order

1. Preserve the manual-only workflow guard and secret checks.
2. Restore the recovery line's faucet, demand-miner, RPC, and supporting test
   slices without deploying them.
3. Run Go, race, Lambda, installer, workflow-contract, and secret-safety tests.
4. Restore Android-only wallet sources and tests from the wallet
   reconciliation head, excluding overlapping deployment and node changes.
