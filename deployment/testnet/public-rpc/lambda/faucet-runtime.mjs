    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) => {
          timeoutId = setTimeout(() => reject(new FaucetError(503, 'faucet dependency timed out')), timeoutMs);
        }),
      ]);
...
export function createRuntimeFaucetHandler({ seeds, fetchImpl = globalThis.fetch, timeoutMs, env = process.env, logger = console, now = Date.now }) {
  ...
  const timed = createOperationTimer({
    logger,
    now,
    timeoutMs: Number.isInteger(dependencyTimeoutMs) && dependencyTimeoutMs >= 100 && dependencyTimeoutMs <= 15_000
      ? dependencyTimeoutMs
      : 6_000,
  });
