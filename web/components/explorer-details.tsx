...
function DetailHeader({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) {
  return (
    <header className="explorer-detail-header">
      ...
      <h1>{title}</h1>
      {subtitle ? <p className="muted mono explorer-break">{subtitle}</p> : null}
  ...
  return (
    <article className="explorer-detail-card">
      ...
      <dl className="explorer-detail-list">
        ...
        <div><dt>Previous hash</dt><dd className="mono explorer-break">{block.previous_hash || "—"}</dd></div>
        ...
        {block.transactions?.length ? (
          <div className="explorer-detail-rows">
            {block.transactions.map((tx) => (
              <Link key={tx.id} className="explorer-detail-row" href={`/explorer/tx?id=${tx.id}`}>
                <strong className="mono explorer-break">{tx.id}</strong>
                <span>{formatSUDH(tx.amount)}</span>
              </Link>
            ))}
          </div>
...
export function ExplorerTransactionDetail({ apiBaseUrl, transactionId }: TransactionDetailProps) {
  ...
  return (
    <article className="explorer-detail-card">
      ...
      {!confirmed ? <p className="notice">This transaction is not yet included in a canonical block.</p> : null}
      <dl className="explorer-detail-list">
        ...
        <div><dt>Amount</dt><dd>{formatSUDH(tx.amount)}</dd></div>
        <div><dt>Fee</dt><dd>{formatSUDH(tx.fee)}</dd></div>
        ...
        {transactions.length ? (
          <div className="explorer-detail-rows">
            {transactions.map((item) => (
              <Link key={`${item.transaction.id}-${item.block_height ?? "pending"}`} className="explorer-detail-row" href={`/explorer/tx?id=${item.transaction.id}`}>
                <strong className="mono explorer-break">{item.transaction.id}</strong>
                <span>{formatSUDH(item.transaction.amount)} · {item.confirmations} conf.</span>
              </Link>
            ))}
          </div>
