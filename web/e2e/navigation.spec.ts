...
test("Downloads exposes only official synchronized public release assets", async ({ page }) => {
  ...
  expect(walletArtifact?.checksumUrl).toBe("/downloads/Sudharma-Wallet-latest.apk.sha256");
  ...
  await expect(walletCard.getByRole("link", { name: "Download" })).toHaveAttribute("href", "/downloads/Sudharma-Wallet-latest.apk");
  await expect(walletCard.getByRole("link", { name: "Download" })).not.toHaveAttribute("target", "_blank");
