...
func TestEmptyBlockBroadcastBetweenNodes(t *testing.T) {
	...
	block :=
		&blockchain.Block{
			...
			Nonce:        0,
			MinerAddress: "test-miner-address",
			Transactions: nil,
		}
	...
	// Commit block locally on node B first.
	if err := chainB.AddBlock(
		block,
	); err != nil {
		t.Fatal(err)
	}
	...
	if chainA.Tip().Hash() !=
		...
		t.Fatal(
			"node A tip does not match broadcast block",
		)
	}

	if nodeA.Height != 1 {
		t.Fatalf(
			"expected node A advertised height 1, got %d",
			nodeA.Height,
		)
	}
}
