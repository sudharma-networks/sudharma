...
func TestEmptyBlockBroadcastBetweenNodes(t *testing.T) {
	...
	if err := chainB.AddBlock(block); err != nil {
		t.Fatal(err)
	}
	nodeB.RefreshChainStatus()
	if err := nodeB.BroadcastBlock(block); err != nil {
		t.Fatal(err)
	}

	deadline := time.Now().Add(2 * time.Second)
	for chainA.Height() == 0 && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	if chainA.Height() != 1 {
		t.Fatalf("expected node A height 1, got %d", chainA.Height())
	}
	if chainA.Tip().Hash() != block.Hash() {
		t.Fatal("node A tip does not match broadcast block")
	}
	height, _ := nodeA.AdvertisedChainStatus()
	if height != 1 {
		t.Fatalf("expected node A advertised height 1, got %d", height)
	}
}
