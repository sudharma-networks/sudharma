...
func TestMalformedInboundHandshakeIsRejectedAndCleanedUp(t *testing.T) {
	...
	if node.PeerCount() != 0 {
		t.Fatalf("malformed handshake admitted peer: %d", node.PeerCount())
	}
	_ = conn.SetReadDeadline(time.Now().Add(500 * time.Millisecond))
	var b [1]byte
	if _, err := conn.Read(b[:]); err == nil {
		t.Fatal("malformed handshake connection remained open")
	}
}
...
func TestTruncatedInboundHandshakeIsRejectedAndCleanedUp(t *testing.T) {
	...
	waitForInboundHandshakes(t, node, 1)
	if _, err := conn.Write([]byte(`{"type":"handshake"`)); err != nil {
		t.Fatal(err)
	}
	...
	waitForInboundHandshakes(t, node, 0)
	if node.PeerCount() != 0 {
		t.Fatalf("truncated handshake admitted peer: %d", node.PeerCount())
	}
	_ = conn.Close()
}
...
func TestInboundHandshakeCapacityRecoversAfterMalformedFlood(t *testing.T) {
	...
	if node.PeerCount() != 0 {
		t.Fatalf("malformed flood admitted peers: %d", node.PeerCount())
	}
}
...
func TestRepeatedConnectDisconnectDoesNotLeaveStalePeers(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		if peer == nil || peer.conn == nil {
			t.Fatalf("missing live peer at iteration %d", i)
		}
		_ = peer.conn.Close()
