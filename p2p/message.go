...
// GetPeersMessage requests a peer's known peer list.
// The payload is intentionally empty.
...
func NewHandshakeMessage(
	...
	if !ok {
		return nil, fmt.Errorf(
			"total work is not a valid integer",
		)
	}
...
func DecodeGetMempool(
	...
	if message.Type != MessageGetMempool {
		return fmt.Errorf(
			"message is not a get_mempool request",
		)
	}
...
func DecodeGetPeers(
	...
	if message.Type != MessageGetPeers {
		return fmt.Errorf(
			"message is not a get_peers request",
		)
	}
...
func NewPeersMessage(
	peers []KnownPeer,
) ([]byte, error) {

	if len(peers) > MaxPeersPerMessage {
		return nil, fmt.Errorf(
			"too many peers in discovery message: %d > %d",
			len(peers),
			MaxPeersPerMessage,
		)
	}
...
func DecodePeers(
	...
	if message.Type != MessagePeers {
		return nil, fmt.Errorf(
			"message is not a peers response",
		)
	}
	...
	for _, peer := range payload.Peers {
		...
		if peer.NodeID == "" {
			return nil, fmt.Errorf(
				"discovered peer node ID cannot be empty",
			)
		}

		if peer.Address == "" {
			return nil, fmt.Errorf(
				"discovered peer %s has empty address",
				peer.NodeID,
			)
		}
...
func DecodeHandshake(
	...
	if handshake.NetworkID != NetworkID {
		return nil, fmt.Errorf(
			"wrong Sudharma Network network: %s",
			handshake.NetworkID,
		)
	}
	...
	if handshake.NodeID == "" {
		return nil, fmt.Errorf(
			"remote node ID cannot be empty",
		)
	}

	if handshake.TotalWork == "" {
		return nil, fmt.Errorf(
			"remote total work cannot be empty",
		)
	}
	...
	if !ok {
		return nil, fmt.Errorf(
			"remote total work is not a valid integer",
		)
	}

	if work.Sign() < 0 {
		return nil, fmt.Errorf(
			"remote total work cannot be negative",
		)
	}
