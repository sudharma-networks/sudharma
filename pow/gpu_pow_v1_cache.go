...
// GPUV1CacheNode is one 64-byte Khushi epoch-cache item.
type GPUV1CacheNode [64]byte

// GPUV1BuildCache builds a deterministic cache from the Sudharma-specific
// epoch seed. nodeCount stays parameterized so compact interoperability tests
// can run without weakening the fixed production verifier cache policy.
