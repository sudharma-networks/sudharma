...
func TestWaitForPendingTransactionsObservesAsyncMempoolArrival(t *testing.T) {
	...
	if err := node.SetChain(chain); err != nil { t.Fatal(err) }
	if err := node.SetState(state); err != nil { t.Fatal(err) }

	sender, err := wallet.NewWallet()
	if err != nil { t.Fatal(err) }
	receiver, err := wallet.NewWallet()
	if err != nil { t.Fatal(err) }
	if err := state.Credit(sender.Address, 50*params.CoinDecimals); err != nil { t.Fatal(err) }
	...
	if !waitForPendingTransactions(node, 250*time.Millisecond) {
		t.Fatal("expected asynchronous mempool transaction to be observed before timeout")
	}
...
func TestRunBlockMiningTestConfirmsMempoolTransaction(t *testing.T) {
	...
	if err := state.Credit(sender.Address, initialBalance); err != nil { t.Fatal(err) }
	...
	if err := node.Mempool().AddTransaction(tx); err != nil { t.Fatal(err) }

	beforeHeight := chain.Height()
	tempDir := t.TempDir()
	if err := runBlockMiningTest(chain, state, node, filepath.Join(tempDir, "chain.json"), filepath.Join(tempDir, "state.json"), minerWallet.Address, 1); err != nil {
		t.Fatalf("transaction-confirming mining failed: %v", err)
	}

	if got := chain.Height(); got != beforeHeight+1 { t.Fatalf("expected height %d, got %d", beforeHeight+1, got) }
	if got := state.Balance(receiver.Address); got != amount { t.Fatalf("expected receiver balance %d, got %d", amount, got) }
	if got := node.Mempool().Count(); got != 0 { t.Fatalf("expected empty mempool after mining, got %d transaction(s)", got) }
	if _, _, found := chain.TransactionByID(tx.ID); !found { t.Fatalf("mined transaction %s was not indexed in the chain", tx.ID) }
}
