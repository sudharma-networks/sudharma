...
func backupWallet() {
	if len(os.Args) != 4 {
		fmt.Println("Usage: sudharma-wallet backup <wallet-file> <backup-file>")
		return
	}
	...
	address, err := wallet.BackupEncrypted(os.Args[2], os.Args[3], password)
	if err != nil {
		fmt.Println("Wallet backup FAILED:", err)
		return
	}
	fmt.Println("Wallet backup verified successfully.")
	fmt.Println("Address:", address)
	fmt.Println("Backup file:", os.Args[3])
	fmt.Println("Keep backup copies offline and separate from the primary computer.")
}

func changeWalletPassword() {
	if len(os.Args) != 3 {
		fmt.Println("Usage: sudharma-wallet passwd <wallet-file>")
		return
	}
	currentPassword, err := readPassword("Current wallet password: ")
	if err != nil {
		fmt.Println("Failed to read password:", err)
		return
	}
	newPassword, err := readNewPasswordWithPrompts("New wallet password: ", "Confirm new wallet password: ")
	if err != nil {
		fmt.Println("Failed to set new password:", err)
		return
	}
	address, err := wallet.ChangeEncryptedPassword(os.Args[2], currentPassword, newPassword)
	if err != nil {
		fmt.Println("Wallet password change FAILED:", err)
		return
	}
	fmt.Println("Wallet password changed successfully.")
	fmt.Println("Address:", address)
	fmt.Println("The wallet address and private key did not change.")
}
